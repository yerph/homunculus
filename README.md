...
- **住在一个网页前端里** —— iMessage 风格的聊天界面，任何浏览器都能打开
...
**核心思路：** 所有渠道的消息最终都进入同一个 Claude Code 的上下文窗口。它能看到来自任何渠道的消息，也能回复到任何渠道。Keepalive 系统会定时唤醒它，让它可以主动找你聊天。
...
| 模块 | 说明 |
|------|------|
| **chat-server** | 网页长对话聊天前端，简洁的消息界面，支持 Markdown 渲染，适合电脑或平板使用 |
| **phone-widget** | 手机造型的小组件，iMessage 风格聊天气泡、表情回应，适合嵌入网页或作为移动端入口 |
...
**问：我可以给 phone-widget 加自定义小应用吗？**

可以。phone-widget 有一套简单的页面/应用导航系统，参照现有的示例即可添加你自己的小应用。
